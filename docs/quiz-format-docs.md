...
``http(s)`` URLs and non-asset paths (e.g. ``dog.jpg``) are stored unchanged. Any **non-URL** value ending in ``/image.webp`` or ``/thumb.webp`` (e.g. ``./media/{asset_…}/image.webp``) is **invalid** in KQF text — use the asset directory form above. ``http(s)`` URLs ending in those filenames remain valid.
